...
## 7. Finite learning-operator domain
...
## 10. Learner-visible evidence object `e_t`
...
\[
\boxed{
\hat k_t=\operatorname{Diagnose}(e_t,S_t)
}
\]
...
For the recursive episode, after all four witness events have been observed, diagnosis additionally evaluates the fixed recurrence condition defined in Section 15. If that condition holds, the episode receives:
...
5. no lower-level state carried under the frozen grammar can alter the dispatch table.
...
## 16. Lower-level exclusion certificate for the recursive witness

The decisive pre-execution certificate is an exhaustive simulation over all lower-level update histories permitted by ​\((\mathcal G,B)\).
...
\quad
h\not\models W_{\mathcal L}^{\rm recurrent\_repair}.
}
\]

Here `models` means that after the complete four-event witness, the trace has both:

1. solved the required fresh `k_F` recurrence behavior; and
2. changed the dispatch relation governing the future `k_F` update target.

Because the lower-level grammar explicitly forbids dispatch mutation, condition 2 cannot be satisfied by any lower-level trace. The exhaustive certificate nevertheless enumerates the full finite lower-level space and records the strongest achieved lower-level repair as a negative control.

The certificate must be generated **before** execution of the recursive treatment.

---

## 17. Operator revision rule
...
## 20. Same-evidence invariant

For every induction event:

\[
\boxed{
 e_t^{(M)}=e_t^{(A)}=e_t^{(F)}=e_t^{(L)}
}
\]
...
## 22. Held-out future curriculum generator
...
## 24. Mechanism and future-benefit separation
...
## 25. Independent correction challenge
...
## 31. Frozen seeds and generators
