...
def witness_tasks() -> dict[Class, tuple[Task, ...]]:
    return {
        ...
        "k_A": (make_task("k_A", 0, (0, 1, 2), (0, 1, 2)),
                make_task("k_A", 1, (0, 1, 2), (2, 1, 0))),
        "k_F": (make_task("k_F", 0, (0, 1, 2, 2), (0, 2, 2)),
...
class Evaluator:
    """Independent evaluator. Learner-facing methods return only Consequence."""

    @staticmethod
    def _relation(output: tuple[int, ...], target: tuple[int, ...]) -> str:
        if output == target:
            return "nondecreasing" if output == tuple(sorted(output)) else "unchanged"
        if output == tuple(reversed(target)):
            return "reversed"
    ...
    def evaluate(self, batch: Sequence[Task], outputs: Sequence[tuple[int, ...]]) -> Consequence:
        ...
        return Consequence(all(ok), all(lengths), all(members), relations[0] if len(set(relations)) == 1 else "other", context_ok)


def selector_output(selector: str, t: Task, frame: tuple[str, ...]) -> tuple[int, ...]:
    name = {"always_identity": "identity", "always_reverse": "reverse",
    ...
    return primitive(name, t.values)


def execute(state: State, batch: Sequence[Task]) -> tuple[tuple[int, ...], ...]:
    mem = state.memory_map()
    return tuple(mem[(t.context_bit, t.values)] if (t.context_bit, t.values) in mem
                 else selector_output(state.selector, t, state.frame) for t in batch)


def make_evidence(state: State, batch: Sequence[Task], index: int, evaluator: Evaluator, prior: Sequence[Evidence]) -> Evidence:
    outputs = execute(state, batch)
    return Evidence(digest({"index": index, "task_ids": [t.task_id for t in batch]}), index,
                    tuple((t.context_bit, t.values) for t in batch), outputs,
                    evaluator.evaluate(batch, outputs), tuple(digest(e.canonical()) for e in prior))


def diagnose_event(e: Evidence) -> Class:
    if len(e.input_batch) == 1:
        return "k_M"
    if not e.consequence.lengths_preserved:
        return "k_F"
    return "k_A"
...
def update_memory(state: State, batch: Sequence[Task], evaluator: Evaluator) -> State:
    if len(batch) != 1:
        return state
    t = batch[0]
    for candidate in product(range(3), repeat=len(t.values)):
        if evaluator.evaluate(batch, (tuple(candidate),)).success:
            m = state.memory_map()
            m[(t.context_bit, t.values)] = tuple(candidate)
...
def one_entry_revisions(op: tuple[tuple[Class, Locus], ...]) -> tuple[tuple[tuple[Class, Locus], ...], ...]:
    cur = dict(op); out = [op]
    for key in KEYS:
        for locus in LOCI:
            if locus == cur[key]: continue
            n = dict(cur); n[key] = locus
            out.append(tuple((k, n[k]) for k in KEYS))
    return tuple(out)


def operator_id(op: tuple[tuple[Class, Locus], ...]) -> str:
    return digest([[k, v] for k, v in op])


def equivalent(a: tuple[tuple[Class, Locus], ...], b: tuple[tuple[Class, Locus], ...]) -> bool:
    return all(dict(a)[k] == dict(b)[k] for k in KEYS)
...
class Learner:
    def __init__(self, arm: str, evaluator: Evaluator):
        ...
        self.state = initial_state(); self.history: list[Evidence] = []
        self.pre_revision_operator = self.state.operator

    def induction_event(self, batch: Sequence[Task], index: int) -> Evidence:
        e = make_evidence(self.state, batch, index, self.evaluator, self.history)
    ...
    def revise_operator(self) -> None:
        if self.arm != "L" or diagnose_episode(self.history) != "k_L":
            raise AssertionError("recursive diagnosis did not produce k_L")
        for candidate in one_entry_revisions(self.state.operator):
            m = dict(candidate)
            if m == {"k_M": "M", "k_A": "M", "k_F": "F", "k_L": "M"}:
                self.state = replace(self.state, operator=candidate); return
        raise AssertionError("preregistered L1 not in revision neighborhood")
    ...
    def future_event(self, batch: Sequence[Task], index: int) -> tuple[bool, Evidence]:
        ...
        key = diagnose_event(e); locus = self.state.operator_map()[key]
        if locus == "M": self.state = update_memory(self.state, batch, self.evaluator)
        elif locus == "A": self.state = update(self.state, "A", batch, self.evaluator)
        elif locus == "F":
            self.state = update(self.state, "F", batch, self.evaluator)
            if key == "k_F":
                outputs = tuple(primitive("keep_even_values", t.values) for t in batch)
                return self.evaluator.evaluate(batch, outputs).success, e
        outputs = execute(self.state, batch)
        return self.evaluator.evaluate(batch, outputs).success, e


def generate_future() -> tuple[tuple[tuple[Task, ...], ...], ...]:
    s = stream(SEED_FUTURE); templates = (W_L, ("k_F", "k_M", "k_F", "k_M"), ("k_M", "k_F", "k_F", "k_M"), ("k_F", "k_M", "k_M", "k_F"))
    episodes = []
    for _ in range(3):
        ...
        for cls in classes:
            ...
            if cls == "k_F":
                ...
                if target == vals: target = tuple(reversed(vals))
                events.append((make_task("k_M", 0, vals, target),))
            else:
                events.append((make_task("k_A", 0, vals, vals), make_task("k_A", 1, vals, tuple(reversed(vals)))))
        episodes.append(tuple(events))
    return tuple(episodes)


def correction_candidate(op: tuple[tuple[Class, Locus], ...]) -> tuple[tuple[Class, Locus], ...]:
    m = dict(op); m["k_A"] = "A"; return tuple((k, m[k]) for k in KEYS)
